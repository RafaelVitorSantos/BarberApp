...
if (password_verify($senha, $dadosValidate['usu_senha'])) {
    if (strtoupper($dadosValidate['usu_email']) == strtoupper($email)) {
        $sqlUpdate = "UPDATE cad_usuario SET usu_nome='$nome', usu_telefone='$telefone' WHERE usu_id = $id";
        ...
        if ($rowEmail == 0) {
            $sqlUpdate = "UPDATE cad_usuario SET usu_nome='$nome', usu_email='$email', usu_telefone='$telefone' WHERE usu_id = $id";
            mysqli_query($connection, $sqlUpdate);
            header("Location: ../../../../client/page/index.php?successEdit");
            die();
        } else {
