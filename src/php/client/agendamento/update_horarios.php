...
if (isset($_POST['servico']) && isset($_POST['data'])) {
    ...
    $intervaloBruto = $dadosConfig['config_intervalo'];
    $intervalo = strtotime($intervaloBruto);
    ...
    while (strtotime($horarioMostrado) < strtotime($horariofinal)) {
        ...
        $sqlValidate = "SELECT * FROM os_agendamento WHERE age_status = 1 AND age_data = '$data' AND age_func_id = '$funcionarioId'";
        ...
        $horarioMostrado = date('H:i', strtotime($horarioMostrado) + $intervalo);
    }
}
